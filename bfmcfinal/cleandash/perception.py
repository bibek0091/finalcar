...
class HybridLaneTracker:
    ...
    # RIGHT_LANE_BIAS_PX: comfort shift rightward so the car stays clear of divier.
    ...
    RIGHT_LANE_BIAS_PX       = 5     # small comfort shift away from right edge
    DIVIDER_FOLLOW_OFFSET_PX = 90   # px right of divider when sr is lost
    ...
    def get_target_x(self, y_eval, lane_width_px, extra_offset_px=0,
        ...
        # 3-TIER RIGHT-LANE PRIORITY SYSTEM
        #
        #  TIER 1 — BOTH LINES visible:
        #    True lane centre (sl+sr)/2. Most accurate.
        #
        #  TIER 1b — RIGHT LINE only (sr visible, sl lost):
        #    Follow sr with a half-lane-width offset to the left so the
        #    car sits in the right lane beside the right edge.
        #    Offset = hw*0.85 so it hugs right without hitting the kerb.
        #
        #  TIER 2 — LEFT LINE only (sl visible, sr lost):
        #    Follow sl with a half-lane-width offset to the right so
        #    the car stays in the right lane beside the centre divider.
        #
        #  TIER 3 — BLIND (both lost):
        #    Dead reckoning + pull toward right lane (320+hw) over ~1.3 s.
        # ══════════════════════════════════════════════════════════════

        # ─ TIER 3: both lost ────────────────────────────────────────
        if not has_right and not has_left:
            predicted_x, conf = self.dead_reckoner.predict_target(last_speed, last_steering)
            # Pull target toward right-lane default (320+hw) over ~1.3 s of blindness
            safe_right_x = 320.0 + hw
            t = self.dead_reckoner._lost_time_s
            pull = min(1.0, t * 0.75)
            base_x = predicted_x * (1.0 - pull) + safe_right_x * pull
            return base_x + extra_offset_px, f"BLIND_KEEP_RIGHT_{conf:.2f}"

        # ─ TIER 1: both lines visible → true lane centre ─────────────
        if has_left and has_right:
            base_x = (ev(sl) + ev(sr)) * 0.5
            anchor = "CENTER_DUAL"

        # ─ TIER 1b: sr only → follow right edge, offset left by hw*0.85 ─
        elif has_right:
            # Sit half a lane-width to the left of the right line so the
            # car is centred in the right lane, not on top of the kerb.
            # RIGHT_LANE_BIAS_PX is a small positive nudge away from the edge.
            base_x = ev(sr) - (hw * 0.85) + self.RIGHT_LANE_BIAS_PX
            anchor = "FOLLOW_RIGHT_LINE"

        # ─ TIER 2: sl only → follow divider, offset right by hw*0.85 ──
        else:
            base_x = ev(sl) + (hw * 0.85) + self.RIGHT_LANE_BIAS_PX
            anchor = "FOLLOW_LEFT_LINE_RIGHT_SIDE"
